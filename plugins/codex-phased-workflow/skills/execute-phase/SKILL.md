...
**Shared conventions:** read `<PLUGIN_ROOT>/refs/common.md` and `<PLUGIN_ROOT>/refs/contracts.md` once at start. Do not load the relay layer here: when a decision must travel, the shared core's *Routing a decision* reaches it; `Channel: in-chat` never loads it. **Shared mechanics:** `<PLUGIN_ROOT>/refs/phase-execution.md` — selection, implementation discipline, outcome formats, the phase commit; `/execute-phase-agent` is this same skill with the gate replaced by unattended constraints.
...
On `Channel: relayed` and legacy plans, title this task `wf:<slug>:phase-N — <phase title>`. On `Channel: in-chat`, title the conversation `wf:<slug>` once and keep it: renaming the same conversation at every phase loses the workflow-level address. Titles are best-effort.
...
for a path the plan names is a plan ambiguity routed before the gate per
*Routing a decision*, not an empty success. **This fan-out writes nothing.**
...
names, the behaviour a later test asserts. A conflict found here is a plan ambiguity routed per *Routing a decision* before any approval is asked — approving an approach nobody checked
against the plan's own future is how a phase betrays it.

**Plan ambiguities are routed before the gate.** On the relayed road, send `clarify?` to the foreman and fold its answer into the gate for confirmation. On `Channel: in-chat`, put the ambiguity directly to the user in this gate. In both cases the covering decision is recorded in `notes.md` under `## Phase N`.
...
longer fits takes the plan's decision route (`contracts.md` → *Verification*,
authored checks are plan-author-owned). Approval of the phase IS approval of the mockup:
...
Implement only this phase. When a coherent, demonstrable sub-result lands and substantial work remains, checkpoint it per the shared core (*WIP checkpoints*) — the cost is a `partial` commit the squash will drop, the payoff is that a dying session loses minutes, not the phase. A phase carrying `> Batches:` instead commits each planned batch per *Planned batches* and continues without a WIP note or handover. If something the plan doesn't cover comes up and a wrong default would be costly, ask ONE batched question and record the answer in Notes; otherwise take the conservative option and note it. Plan ambiguities take the same route mid-phase as at the gate.
...
**When an answer changes the plan itself** — a phase reshaped, a decision reversed, scope moved — commit the plan edit and report it by *Routing a decision*: one best-effort `plan changed at phase N` message on relayed, directly at the gate on in-chat.

## Step 5: Verify

- Phases with contract tests start from them: copied verbatim and green per the shared core (`refs/phase-execution.md` → *Implement*); edits to their contract only arrive through a covering decision on the configured route.
...
- ONE phase commit per phase, at Step 6 — WIP checkpoints and planned batches are `partial` commits, not phase commits
